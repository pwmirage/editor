...
<div id="background" onmousedown="{serialize $win}.activate();" onmouseup="{serialize $win}.tryclose();" oncontextmenu="event.stopPropagation(); return false;" class="{if $bg}visible{/if}">
	<div class="menu">
		{for e of $entries}
			...
			<div id="entry-{@$e.id}" class="entry{if $e.disabled} disabled{/if}{if !$e.id} unclickable{/if}{if !$e.id && !$e.children} text{/if}" onmouseup="event.stopPropagation();{if $e.id && !$e.disabled}{serialize $win}.select({@$e.id});{/if}" onmouseenter="{serialize $win}.hover_entry(this);">
				<span>{@$e.name}</span>
				{if $e.children}
