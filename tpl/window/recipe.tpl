...
<div class="content" style="">
	...
	<div id="targets" class="flex-columns" style="flex-wrap: wrap; column-gap: 5px;">
		{for i = 0; i < 4; i++}
			<div class="target">
				...
				<span class="item menu-triangle" onmousedown="{serialize $win}.item_onclick(event, this, 'targets', {@$i});" ondblclick="{serialize $win}.item_ondblclick(event, this, 'targets', {@$i});" data-id="{@$target.id}" data-idx="{@$i}" tabindex="0"><img{ } src="{@Item.get_icon_by_item(db, $target.id)}" alt=""></span>
	...
	<div id="mats" class="flex-columns" style="flex-wrap: wrap; column-gap: 5px;">
		...
		{for mat of $recipe?.mats}
			...
			{$count++}
			<div class="target">
				<span class="item menu-triangle" onmousedown="{serialize $win}.item_onclick(event, this, 'mats', {@$i});" ondblclick="{serialize $win}.item_ondblclick(event, this, 'mats', {@$i});" data-id="{@$mat.id}" data-idx="{@$i}" tabindex="0"><img{ } src="{@Item.get_icon_by_item(db, $mat.id)}" alt=""></span>
		...
		{for j = $count; j < 8; j++}
			...
			<div class="target">
				<span class="item menu-triangle" onmousedown="{serialize $win}.item_onclick(event, this, 'mats', {@$i});" ondblclick="{serialize $win}.item_ondblclick(event, this, 'mats', {@$i});" data-id="0" data-idx="{@$i}" tabindex="0"><img{ } src="{@Item.get_icon_by_item(db, 0)}" alt=""></span>
				<span data-input class="input-number" style="width: 28px; font-size: 12px; padding: 3px;" data-link="{serialize $recipe} => 'mats', {@$i}, num" data-placeholder="(unnamed)"></span>
			</div>
		{/for}
	</div>
